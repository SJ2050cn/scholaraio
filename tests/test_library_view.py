...
def test_main_library_detail_returns_abstract_conclusion_toc_and_commands(tmp_path: Path) -> None:
    ...
    assert detail["commands"]["show_l4"] == 'scholaraio show "paper-detail" --layer 4'
    assert detail["commands"]["repair"].startswith('scholaraio repair "paper-detail"')
...
def test_proceedings_detail_returns_volume_context_and_commands(tmp_path: Path) -> None:
    ...
    assert detail["commands"]["search"] == 'scholaraio fsearch "Wave proceedings paper" --scope proceedings'
